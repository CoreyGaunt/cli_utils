"""This module provides functions to execute
shell commands using subprocess."""
...
@click.option('--downstream', '-d', default='', is_flag=False, 
              ...
              help=("Run specified model & its children models. You can also "
                    ...
                    "or <model_name>+2. Defaults to <model_name>+."))
@click.option('--waterfall', '-a', is_flag=True,
              help=("Run the specified model, its children models, and the parents "
                    "of its children models. Leverages the '@' dbt operator. NOTE - "
                    "This command cannot be run alongside --upstream or --downstream."))
def cli(prod, upstream, downstream, waterfall):
    """
    Run dbt models in the local environment, or the production environment if 
    the --prod flag is set.

    Production access is granted by Analytics Engineering. Please reach out to 
    the team if you need access.

    This command will scan the models directory for .sql files and prompt you 
    to select a model to run. You can also specify the number of upstream or 
    downstream models to run alongside the selected model. If the --waterfall 
    flag is set, it will run the selected model, its children models, and the 
    parents of its children models.

    When the command completes (whether successful or not), it will add the 
    generated bash command to your zsh history.
    """
    prefix, suffix = '', ''

    if waterfall:
        if upstream or downstream:
            console.print("Cannot run --waterfall with --upstream or --downstream",
                          style="bold red")
            exit()
        prefix = '@'
    else:
    ...
    model_name = utils.gum_filter(model_list, "Select A Model To Run", False)
    model_string = ''
    for model in model_name.split():
        model_string += f"{prefix}{model}{suffix} "
    model_string = model_string.strip()

    use_venv = "cd dbt && source venv/bin/activate && /bin/bash"
    ...
    if prod:
        ...
        if not prod_password:
            ...
            exit()
        cmd = f"{use_venv} dbt run -s {model_string} --target prod"
    else:
