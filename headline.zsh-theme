...
HEADLINE_GIT_PREFIX_CMD='git('
HEADLINE_GIT_BRANCH_CMD='headline_git_branch'
HEADLINE_GIT_SUFFIX_CMD=')'
...
HEADLINE_PATH_TO_BRANCH=' on ' # only used when no padding between <path> and <branch>
HEADLINE_PATH_TO_PAD='' # used if padding between <path> and <branch>
HEADLINE_PAD_TO_BRANCH_PREFIX=''
HEADLINE_PAD_TO_BRANCH='' # used if padding between <path> and <branch>
HEADLINE_BRANCH_TO_BRANCH_SUFFIX=''
...
HEADLINE_STYLE_BRANCH_PREFIX_LINE=$HEADLINE_STYLE_BRANCH_PREFIX
...
headline_precmd() {
  ...
  # Information
  local user_str host_str path_str branch_prefix branch_str branch_suffix status_str
  user_str=$(eval $HEADLINE_USER_CMD)
  host_str=$(eval $HEADLINE_HOST_CMD)
  path_str=$(eval $HEADLINE_PATH_CMD)
  branch_prefix=$HEADLINE_GIT_PREFIX_CMD
  branch_str=$(eval $HEADLINE_GIT_BRANCH_CMD)
  branch_suffix=$HEADLINE_GIT_SUFFIX_CMD
...
#   fi
  ...
  if (( ${#branch_prefix} && ${#branch_str})); then
	_headline_part JOINT "$HEADLINE_PATH_TO_BRANCH" left
	_headline_part BRANCH_PREFIX "$HEADLINE_BRANCH_PREFIX$branch_prefix" left
  fi

  # Git branch
  local len=$(( $_HEADLINE_LEN_REMAIN - ${#HEADLINE_BRANCH_PREFIX} ))
  if (( ${#branch_str} )); then
    if (( $len < ${#HEADLINE_PATH_PREFIX} + ${#HEADLINE_TRUNC_PREFIX} + ${#HEADLINE_PATH_TO_BRANCH} + ${#branch_prefix} + ${#branch_str} + ${#branch_suffix})); then
      path_str=''
    fi
    if (( ${#path_str} )); then
    ...
    _headline_part BRANCH "%$len<$HEADLINE_TRUNC_PREFIX<$branch_str%<<" left
	# _headline_part BRANCH_SUFFIX "$branch_suffix" right
  fi

  # Git suffix
  local len=$(( $_HEADLINE_LEN_REMAIN - ${#HEADLINE_BRANCH_PREFIX} ))
  if (( ${#branch_suffix} && ${#branch_str})); then
	_headline_part BRANCH_PREFIX "$branch_suffix" left
  fi

  # Padding
  if (( ${#branch_prefix} && ${#branch_str} && ${#branch_suffix} && ${#path_str} && $_HEADLINE_LEN_REMAIN <= ${#HEADLINE_PATH_TO_BRANCH} )); then
    _headline_part JOINT "$HEADLINE_PATH_TO_BRANCH" left
  else
    if (( ${#branch_prefix} && ${#branch_str} && ${#branch_suffix} )); then
